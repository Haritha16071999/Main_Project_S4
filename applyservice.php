...
$id=$_GET['id'];
$uid = $_SESSION['RID'];
$sql2= mysqli_query($conn,"SELECT * FROM `tbl_service` WHERE `service_id`='$id'");
...
$sql = "INSERT INTO `tbl_servicesubmission`( `login_id`, `document`, `sts`, `service_id`) VALUES ('$uid','$fileName','pending','$id')";
$result = mysqli_query($conn,$sql);
if($result){
echo '<script>alert("Successfully Applied")</script>'; 
header('applyservice.php');
}else{
